...
const Login = ({ onLogin, logos }: { onLogin: (role: 'admin' | 'student', data?: any) => void, logos: { logo1: string, logo2: string } }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setError('');
    try {
      console.log('Attempting login to:', window.location.origin + '/api/login');
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      ...
      {/* Mobile Navigation */}
      <nav className="lg:hidden bg-white border-b border-slate-100 px-4 py-2 flex overflow-x-auto no-scrollbar gap-2 sticky top-[88px] z-30">
        {[
          { id: 'home', label: 'Home', icon: Home },
          { id: 'news', label: 'News', icon: Newspaper },
          { id: 'officers', label: 'Officers', icon: Users },
              ...
              <div className="border-t border-slate-100 pt-10">
                                  ...
                                  <div className="flex items-center gap-4">
                                    ...
                                    {c.image_url && (
                                      <img 
                                        src={c.image_url} 
                                        alt={c.name} 
                                        className="w-12 h-12 rounded-xl object-cover border-2 border-white/20 cursor-zoom-in" 
                                        referrerPolicy="no-referrer"
                ...
                <div className="mb-12">
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-5 gap-4">
                    {Array.from(new Set(stats.voters.map((v: any) => v.year))).sort().map(year => {
                      if (!year) return null;
                      const gradeVoters = stats.voters.filter((v: any) => v.year === year);
                      ...
                      return (
                        <div key={year} className="p-4 bg-slate-50 rounded-2xl border border-slate-50 text-center">
...
const CandidateForm = ({ onComplete, initialData }: { onComplete: () => void, initialData?: Candidate }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const url = initialData?.id ? `/api/candidates/${initialData.id}` : '/api/candidates';
    const method = initialData?.id ? 'PUT' : 'POST';
...
const PositionManager = ({ onUpdate }: { onUpdate: () => void }) => {
  ...
  const fetchPositions = () => fetch('/api/positions').then(res => res.json()).then(data => {
  ...
  const handleMove = async (id: number, direction: 'up' | 'down') => {
    ...
    await Promise.all([
      fetch(`/api/positions/${currentPos.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ order_index: otherPos.order_index })
      }),
      fetch(`/api/positions/${otherPos.id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
